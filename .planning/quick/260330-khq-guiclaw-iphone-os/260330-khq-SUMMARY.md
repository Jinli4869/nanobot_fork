...
plan: 01
subsystem: guiclaw-ios-backend
tags: [ios, wda, guiclaw, backend, normalization, routing]
dependency_graph:
  requires: [guiclaw/backends/adb.py, guiclaw/interfaces.py, guiclaw/skills/normalization.py]
  provides: [guiclaw/backends/ios_wda.py, iOS-normalization, gui.ios-routing]
  affects: [guiclaw/cli.py, nanobot/agent/router.py, nanobot/agent/loop.py, nanobot/agent/capabilities.py]
tech_stack:
...
key_files:
  created: [guiclaw/backends/ios_wda.py]
  modified:
...
decisions:
  ...
  - "iOS app annotation via annotate_ios_apps() handled in guiclaw/prompts/system.py alongside android branch, not in cli.py"
...
# Phase quick-260330-khq Plan 01: iOS WDA Backend for GUIClaw Summary
...
Added complete iOS/iPhone device control to GUIClaw, mirroring the existing ADB Android backend pattern.

### Task 1: WdaBackend and iOS App Normalization

**`guiclaw/backends/ios_wda.py`** (~260 lines, new file):
...
**`guiclaw/skills/normalization.py`** additions:
...
- **Issue:** `import wda` at module level caused `ImportError` since `facebook-wda` is not installed in the dev environment; this would break all GUIClaw imports on non-iOS hosts
- **Fix:** Added `_import_wda()` helper with lazy import + helpful error message; called only in `WdaBackend.__init__`
- **Files modified:** `guiclaw/backends/ios_wda.py`
- **Commit:** 1f22c8f
...
- **Issue:** The plan said to add iOS app annotation in `cli.py`, but the actual annotation logic is in `guiclaw/prompts/system.py` which already has an android branch. Adding it to cli.py would have duplicated logic and missed the system prompt path used by both CLI and nanobot GUI agent
- **Fix:** Added `elif platform == "ios"` branch in `guiclaw/prompts/system.py` alongside the android branch — consistent with existing architecture
- **Files modified:** `guiclaw/prompts/system.py`
- **Commit:** 34adab0
...
- `guiclaw/backends/ios_wda.py` exists (260+ lines, WdaBackend class, all action types)
- `guiclaw/skills/normalization.py` updated with `_IOS_BUNDLE_DISPLAY_NAMES`, `annotate_ios_apps`, `resolve_ios_bundle`
- `guiclaw/cli.py` has `IosConfig`, `ios` choice, `WdaBackend` construction, `ios_default` cache key
