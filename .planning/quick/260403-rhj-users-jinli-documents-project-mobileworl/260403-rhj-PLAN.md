# Quick Task 260403-rhj: 根据 /Users/jinli/Documents/Project/MobileWorld/src/mobile_world/agents/implementations 里的 general_e2e、qwen3vl、mai_ui、gelab、seed agent，为 guiclaw 适配不同 agent 的动作空间和 prompt
...
Add an explicit GUIClaw agent-profile layer so selected MobileWorld-style agent variants can customize prompt format, action schema, response parsing, and coordinate handling without branching the main `GuiAgent` loop.

## Tasks

### Task 1
- files: `guiclaw/agent.py`, `guiclaw/prompts/system.py`, `guiclaw/interfaces.py`, `guiclaw/cli.py`
...
### Task 2
- files: `tests/test_guiclaw.py`, `tests/test_guiclaw_p5_cli.py`
...
- done: Regression coverage demonstrates each supported profile maps MobileWorld-style output into GUIClaw `Action`s.
