...
def _auto_accept_candidate(
    ...
    *,
    ...
    for skill, score in candidates:
        if score < threshold:
            continue
        feedback = feedback_by_id.get(getattr(skill, "skill_id", ""), {})
        if (
            feedback_task_similarity(task, feedback.get("negative_tasks")) >= 0.72
            and feedback_task_similarity(task, feedback.get("preferred_for_tasks")) < 0.72
        ):
            continue
        return skill, score
    return None
