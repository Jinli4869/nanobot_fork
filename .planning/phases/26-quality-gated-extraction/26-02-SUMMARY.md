...
dependency_graph:
  ...
  affects: [guiclaw/skills/__init__.py, phase-26-verification]
tech_stack:
  added: []
  patterns:
...
decisions:
  ...
  - "Phase 26 symbols are exported from guiclaw.skills so callers can adopt the new pipeline without deep module imports."
metrics:
...
**One-liner:** Added `ExtractionPipeline` to orchestrate step and trajectory critics in order, then exported the full Phase 26 surface from `guiclaw.skills`.

## What Was Built

- Added `_AlwaysPassStepCritic`, `_AlwaysPassTrajectoryCritic`, and `ExtractionPipeline` to `guiclaw/skills/shortcut_extractor.py`.
...
- Exported `ExtractionPipeline`, verdict/result types, critic protocols, and `ShortcutSkillProducer` from `guiclaw.skills`.
...
- `guiclaw/skills/shortcut_extractor.py` contains `_AlwaysPassStepCritic`, `_AlwaysPassTrajectoryCritic`, and `ExtractionPipeline`
- `ExtractionPipeline.run()` returns `too_few_steps`, `step_critic`, and `trajectory_critic` rejections on the expected branches
- `guiclaw/skills/__init__.py` re-exports all eight Phase 26 public types
