...
class SkillsLoader:
    ...
    def list_skills(
        ...
        if gui_backend:
            skills = [
                s
                for s in skills
                if self._check_gui_backend(self._get_skill_meta(s["name"]), gui_backend)
            ]
    ...
    def build_skills_summary(
        ...
        Args:
            ...
            gui_backend: Optional GUI backend key used to filter backend-specific skills.
    ...
    def get_always_skills(self, gui_backend: str | None = None) -> list[str]:
        ...
        return [
            entry["name"]
            for entry in self.list_skills(filter_unavailable=True, gui_backend=gui_backend)
