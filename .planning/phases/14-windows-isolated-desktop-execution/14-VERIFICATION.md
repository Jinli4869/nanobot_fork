...
human_verification:
  ...
  - test: "Cleanup leak check on success, startup failure, and cancellation"
    expected: "Logs include cleanup_reason=normal/startup_failed/cancelled and no stale GUIClaw desktops or child processes remain."
...
| --- | --- | --- | --- |
| 1 | Supported Windows runs launch automation inside an isolated desktop/session target. | ✓ VERIFIED | `Win32DesktopManager.start()` now creates a real desktop handle via `_win32_create_desktop()` and `stop()` closes it via `_win32_close_desktop()` in `guiclaw/backends/displays/win32desktop.py:97` and `guiclaw/backends/displays/win32desktop.py:117`. `launch_windows_worker()` sets `STARTUPINFO.lpDesktop` in `guiclaw/backends/windows_worker.py:62`, and the worker services `observe` / `execute` / `list_apps` / `shutdown` in `guiclaw/backends/windows_worker.py:88` through `guiclaw/backends/windows_worker.py:126`. `WindowsIsolatedBackend` routes `observe()`, `execute()`, and `list_apps()` through worker RPC instead of the parent backend in `guiclaw/backends/windows_isolated.py:108`, `guiclaw/backends/windows_isolated.py:129`, and `guiclaw/backends/windows_isolated.py:146`. Regression coverage exists in `tests/test_guiclaw_p14_windows_desktop.py:148`, `tests/test_guiclaw_p14_windows_desktop.py:192`, `tests/test_guiclaw_p14_windows_desktop.py:305`, and the full slice passed: `50 passed in 4.16s`. |
| 2 | Unsupported launch contexts or incompatible app classes are blocked or warned explicitly. | ✓ VERIFIED | `probe_windows_isolated_desktop_support()` rejects unsupported app classes and non-interactive/input-desktop/create-desktop failures in `guiclaw/backends/displays/win32desktop.py:33` through `guiclaw/backends/displays/win32desktop.py:66` and `guiclaw/backends/displays/win32desktop.py:135` through `guiclaw/backends/displays/win32desktop.py:206`. CLI forwarding is wired through `--target-app-class`, `resolve_target_app_class()`, and `probe_isolated_background_support(..., target_app_class=...)` in `guiclaw/cli.py:225`, `guiclaw/cli.py:280`, and `guiclaw/cli.py:458`. Nanobot exposes the same signal in `nanobot/agent/tools/gui.py:91`, forwards it in `nanobot/agent/tools/gui.py:132` through `nanobot/agent/tools/gui.py:140`, and defaults omitted Windows background-local runs to `classic-win32` in `nanobot/agent/tools/gui.py:326` through `nanobot/agent/tools/gui.py:339`. Host-entry regressions exist in `tests/test_guiclaw_p5_cli.py:1196`, `tests/test_guiclaw_p5_cli.py:1261`, `tests/test_guiclaw_p11_integration.py:471`, and `tests/test_guiclaw_p11_integration.py:533`. |
| 3 | Cleanup closes isolated-desktop resources on success, failure, and cancellation. | ✓ VERIFIED | `WindowsIsolatedBackend.shutdown()` now sends worker `shutdown`, waits/stops the worker, closes pipes, unlinks the control path, then stops the desktop manager and releases the runtime lease in `guiclaw/backends/windows_isolated.py:154` through `guiclaw/backends/windows_isolated.py:225` and `guiclaw/backends/windows_isolated.py:242` through `guiclaw/backends/windows_isolated.py:343`. `Win32DesktopManager.stop()` owns real-handle teardown in `guiclaw/backends/displays/win32desktop.py:110` through `guiclaw/backends/displays/win32desktop.py:123`. Cleanup ordering is covered in `tests/test_guiclaw_p14_windows_desktop.py:398` and nanobot failure propagation is covered in `tests/test_guiclaw_p11_integration.py:564`. |
| 4 | Background-run traces expose enough metadata to diagnose target-surface ownership issues. | ✓ VERIFIED | Ready, worker-launch, and cleanup logs include `backend_name`, `display_id`, `desktop_name`, `lpDesktop`, and `cleanup_reason` in `guiclaw/backends/windows_isolated.py:80` through `guiclaw/backends/windows_isolated.py:87`, `guiclaw/backends/windows_isolated.py:205` through `guiclaw/backends/windows_isolated.py:213`, and `guiclaw/backends/windows_isolated.py:163` through `guiclaw/backends/windows_isolated.py:177`. Host-facing assertions exist in `tests/test_guiclaw_p5_cli.py:1366` and `tests/test_guiclaw_p11_integration.py:564`. |
...
| --- | --- | --- | --- |
| `guiclaw/backends/displays/win32desktop.py` | Real Win32 desktop lifecycle manager and support probe | ✓ VERIFIED | Owns Win32 wrappers, session/input/create-desktop checks, stable desktop naming, and real handle teardown. |
| `guiclaw/backends/windows_worker.py` | Worker process bound to `lpDesktop` with live command loop | ✓ VERIFIED | Launches with `lpDesktop`, keeps stdio pipes open, and services `observe`, `execute`, `list_apps`, and `shutdown`. |
| `guiclaw/backends/windows_isolated.py` | Windows isolated execution seam routing desktop IO through worker RPC | ✓ VERIFIED | Preflight owns lease plus desktop start, IO stays off the parent backend, and shutdown is worker-first then desktop-stop. |
| `guiclaw/backends/background_runtime.py` | Shared Windows reason-code/remediation vocabulary and backend dispatch | ✓ VERIFIED | Windows probe branch, reason codes, and remediation strings are present and host-facing. |
| `guiclaw/cli.py` | CLI app-class propagation into the shared Windows probe | ✓ VERIFIED | Adds `--target-app-class`, Windows-only defaulting, and probe forwarding before mode resolution. |
...
| `tests/test_guiclaw_p14_windows_desktop.py` | Phase 14 code-level regression coverage for handle ownership, worker RPC, routing, and cleanup | ✓ VERIFIED | The key gap-closing tests exist and passed in the 50-test phase slice. |
...
| --- | --- | --- | --- | --- |
| `background_runtime._probe_windows_isolated_support` | `win32desktop.probe_windows_isolated_desktop_support` | Shared Windows capability probe | ✓ WIRED | `guiclaw/backends/background_runtime.py:247` through `guiclaw/backends/background_runtime.py:262` imports and forwards `target_app_class` into the Win32 probe. |
| `Win32DesktopManager.desktop_name` | `windows_worker.launch_windows_worker` | `STARTUPINFO.lpDesktop` at child-process creation | ✓ WIRED | `guiclaw/backends/windows_isolated.py:192` through `guiclaw/backends/windows_isolated.py:197` forwards `desktop_name`; `guiclaw/backends/windows_worker.py:62` binds `WinSta0\\{desktop_name}`. |
| `WindowsIsolatedBackend.observe/execute/list_apps` | `windows_worker` command loop | JSON-line worker RPC via `_send_worker_command()` | ✓ WIRED | `guiclaw/backends/windows_isolated.py:108`, `guiclaw/backends/windows_isolated.py:129`, and `guiclaw/backends/windows_isolated.py:146` send commands consumed by `guiclaw/backends/windows_worker.py:110` through `guiclaw/backends/windows_worker.py:126`. |
| `WindowsIsolatedBackend.shutdown` | `Win32DesktopManager.stop` | Worker-first cleanup ordering | ✓ WIRED | `guiclaw/backends/windows_isolated.py:219` sends worker shutdown before `guiclaw/backends/windows_isolated.py:244` stops the desktop manager. |
| `cli.run_cli` | `probe_isolated_background_support` | CLI forwards Windows `target_app_class` into shared probe | ✓ WIRED | `guiclaw/cli.py:458` through `guiclaw/cli.py:462` forwards the resolved value. |
...
No blocker or warning-level stub patterns were found in the inspected Phase 14 implementation files. The only `rg` matches for empty returns were benign helper returns in `guiclaw/cli.py`.
