...
plan: 02
subsystem: api
tags: [fastapi, pydantic, nanobot-tui, operations, guiclaw, task-launch]
requires:
  - phase: 19-operations-console
    provides: runtime status DTOs, app-local operations registry, run_id-addressed inspection
provides:
  - typed launch request DTOs for the allowed nanobot and GUIClaw operations
  - async /tasks/runs launches backed by the shared operations registry
  - tui-local nanobot and GUIClaw adapters without CLI subprocess execution
affects: [19-03, operations-console, web-task-launch, runtime-status]
tech-stack:
  added: []
  patterns: [discriminated launch allowlist, registry-backed async task dispatch, tui-local GUIClaw adapters]
key-files:
...
key-decisions:
  ...
  - "GUIClaw-backed launches use tui-local backend adapters for local and dry-run execution instead of shelling out through guiclaw.cli."
...
**Typed browser-safe task launches for nanobot URL/settings flows and allowlisted GUIClaw app/settings actions, all returning immediate run ids through the shared registry**
...
## Accomplishments
- Added discriminated task-launch DTOs that only admit `nanobot_open_url`, `nanobot_open_settings`, `guiclaw_launch_app`, and `guiclaw_open_settings`.
...
- Kept browser-facing launch behavior under `nanobot/tui` by translating nanobot requests privately to `GuiSubagentTool` and handling GUIClaw requests through local or dry-run backend adapters instead of CLI subprocesses.
...
- `nanobot/tui/services/tasks.py` - Registry-backed launch orchestration plus private nanobot and GUIClaw adapters.
...
- Used direct GUIClaw backend actions for the allowlisted browser operations so the web surface reuses GUIClaw behavior without becoming a generic command runner.
...
- **Files modified:** None
- **Verification:** `.venv/bin/python -m pytest tests/test_tui_p19_tasks.py tests/test_tui_p19_runtime.py tests/test_guiclaw_p3_nanobot.py tests/test_guiclaw_p16_host_integration.py -q`
