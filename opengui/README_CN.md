...
| Android | `adb` | 屏幕观察使用 scrcpy 帧源，因此需要安装 `demo-live` extra（或自行提供 `Pillow` + `py-scrcpy-sdk`）。文字输入请安装 [ADBKeyboard](https://github.com/senzhk/ADBKeyboard)：`adb shell ime set com.android.adbkeyboard/.AdbIME`。如果 ADBKeyboard 广播输入不可用，OpenGUI 也会自动把内置 `yadb` 下发到设备后再走 `yadb` 输入链路。 |
