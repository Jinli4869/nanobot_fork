...
must_haves:
  truths:
    - "Nanobot GUI tasks build an GUIClaw MemoryRetriever when gui.embedding_model is configured"
    - "The retriever reads from ~/.guiclaw/memory so existing user-maintained memory files are reused"
  ...
  artifacts:
    ...
    - path: "tests/test_guiclaw_p6_wiring.py"
      provides: "Regression coverage for default memory dir loading and GuiAgent injection"
      contains: "test_gui_tool_builds_memory_retriever_from_default_guiclaw_dir"
---

<objective>
Wire Nanobot's gui_task path to use GUIClaw memory retrieval, matching the user's existing
memory location under ~/.guiclaw/memory whenever gui.embedding_model is configured.
...
<task type="auto">
  <name>Add GUIClaw memory retriever wiring to GuiSubagentTool</name>
  <files>nanobot/agent/tools/gui.py</files>
  <action>
Add a helper that builds MemoryStore + MemoryRetriever from ~/.guiclaw/memory using the
...
- Fail soft: if initialization breaks, log a warning and continue without memory instead of blocking GUI tasks
  ...
  <verify>
    <automated>tests/test_guiclaw_p6_wiring.py exercises both retriever construction and GuiAgent injection</automated>
  </verify>
</task>

<task type="auto">
  <name>Add regression tests for the new wiring seam</name>
  <files>tests/test_guiclaw_p6_wiring.py</files>
  <action>
Add one test to verify the default memory directory (~/.guiclaw/memory) is used to build a
MemoryRetriever, and another test to verify the resulting retriever is passed through to
GuiAgent during _run_task().
  </action>
  <verify>
    <automated>uv run pytest tests/test_guiclaw_p6_wiring.py tests/test_guiclaw_p8_trajectory.py tests/test_guiclaw_p3_nanobot.py tests/test_guiclaw_p11_integration.py</automated>
  </verify>
</task>
