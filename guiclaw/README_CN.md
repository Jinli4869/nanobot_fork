...
GUIClaw 是一个基于视觉的 GUI 自动化引擎。它对当前屏幕截图，交由大模型决定下一步操作，执行后再次截图，如此循环直到任务完成。支持 Android（通过 ADB）、iOS（通过 WebDriverAgent）、鸿蒙 OS（通过 HDC）、桌面端（macOS / Linux / Windows）以及 dry-run 测试模式。

GUIClaw 有两种使用方式：

- **独立 CLI** — 直接运行 `guiclaw` 命令，搭配 YAML 配置文件
- **nanobot GUI 子智能体** — GUIClaw 作为工具内嵌于 nanobot，可通过任意 nanobot 渠道（TUI、Gateway、MCP 等）调用
...
1. [安装](#安装)
2. [独立 CLI](#独立-cli)
   - [配置文件](#配置文件-homeguiclaw-configyaml)
   - [命令行参数](#命令行参数)
   - [使用示例](#使用示例)
   - [各平台注意事项](#各平台注意事项)
...
`guiclaw` 命令可在无需启动 nanobot 的情况下独立执行 GUI 任务。
...
```bash
# 控制本地桌面（默认）
guiclaw "打开浏览器，访问 github.com"

# 通过 ADB 控制 Android 设备
guiclaw --backend adb "打开设置，开启 Wi-Fi"

# 通过 WebDriverAgent 控制 iOS 设备
guiclaw --backend ios "打开设置，查看 iOS 版本"

# 通过 HDC 控制鸿蒙 OS 设备
guiclaw --backend hdc "打开设置，开启蓝牙"

# 在 Linux CI 上无头运行
guiclaw --backend local --background "对桌面截图"

# Dry-run 模式（不执行真实操作，用于测试配置）
guiclaw --dry-run "点击保存按钮"
...
如果你会长期使用同一种 profile，建议写进 `~/.guiclaw/config.yaml`：
...
| Android | `adb` | 屏幕观察使用 scrcpy 帧源，因此需要安装 `demo-live` extra（或自行提供 `Pillow` + `py-scrcpy-sdk`）。文字输入请安装 [ADBKeyboard](https://github.com/senzhk/ADBKeyboard)：`adb shell ime set com.android.adbkeyboard/.AdbIME`。如果 ADBKeyboard 广播输入不可用，GUIClaw 也会自动把内置 `yadb` 下发到设备后再走 `yadb` 输入链路。 |
...
使用 nanobot 时，GUIClaw 作为 `gui` 子智能体工具运行。主智能体可将"打开浏览器登录"等任务自动委托给 GUIClaw 执行。
...
当 GUIClaw 通过 nanobot 的 `gui` 工具运行时，请把 profile 配到 `gui.agentProfile`：
...
```json
"gui": { "backend": "adb"        }   // Android 设备（scrcpy 帧 + ADB 操作）
...
guiclaw --backend hdc     "..."    # 鸿蒙 OS
guiclaw --backend local   "..."    # 桌面
guiclaw --backend dry-run "..."    # 测试
```
...
> 对于 GPU 加速或 DirectX 应用，可通过 `guiclaw` CLI 的 `--target-app-class directx`（或 `gpu-heavy`、`electron-gpu`、`uwp`）提示后端处理方式。此选项暂未在 nanobot config.json 中开放。
...
> 5. 如果 ADBKeyboard 广播输入不可用，GUIClaw 会自动把内置 `yadb` 推送到 `/data/local/tmp/yadb`。
...
> 5. 如需转发 MJPEG 视频流：`iproxy 9100 9100`
...
`demo/` sidecar 可以在 nanobot 运行任务时展示实时设备画面，并同步展示
agent 输出、`gui_task` 工具调用、GUIClaw 动作和最终 trace 元数据。静态 demo
轨迹仍然保留。
...
iOS 需要先启动 WDA；真机通常需要转发 WDA 的 `8100` 和 MJPEG 的 `9100` 端口，然后验证：
...
GUIClaw 在 `~/.guiclaw/memory/` 下维护一个持久化知识库（CLI 和 nanobot 共用同一路径）。记忆条目教会智能体如何在你的操作系统环境中导航、与特定应用交互、识别图标，以及遵守你设定的行为策略。
...
可直接编辑 `~/.guiclaw/memory/` 下的 Markdown 文件来增删改条目。规则：
...
| 后端 | 支持平台 | 说明 |
|------|---------|------|
| `local` | macOS / Linux / Windows | 控制运行 nanobot/guiclaw 的本机桌面 |
