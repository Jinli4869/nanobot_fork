...
|----|-------------|-----------------|
| AGENT-04 | `GuiAgent.run()` integrates memory retrieval into system prompt | `guiclaw/agent.py` `_retrieve_memory()` and `_build_messages()`; `tests/test_guiclaw_p2_integration.py::test_memory_injected_into_system_prompt` |
| AGENT-05 | `GuiAgent.run()` integrates skill search -> execute matched skill or free explore | `guiclaw/agent.py` `_search_skill()` and `run()` skill branch; `tests/test_guiclaw_p2_integration.py::test_skill_path_chosen_above_threshold`; `::test_free_explore_when_no_skill_match` |
| AGENT-06 | `GuiAgent.run()` records trajectory via `TrajectoryRecorder` | `guiclaw/agent.py` `start()`, `record_step()`, `finish()` calls; `tests/test_guiclaw_p2_integration.py::test_trajectory_recorded_on_run` |
| MEM-05 | Memory context formatted and injected into system prompt | `guiclaw/agent.py` `_retrieve_memory()` plus `build_system_prompt(..., memory_context=...)`; `tests/test_guiclaw_p2_memory.py` |
| SKILL-08 | Skill execution integrated into agent loop | `guiclaw/agent.py` `self._skill_executor.execute(skill)` branch; `tests/test_guiclaw_p2_integration.py::test_skill_path_chosen_above_threshold` |
| TRAJ-03 | Trajectory recording integrated into agent loop | `guiclaw/agent.py` phase changes and `record_step()`; `tests/test_guiclaw_p2_integration.py::test_trajectory_recorded_on_run` |
| TEST-05 | Full agent loop integration test with DryRunBackend + mock LLM + memory + skills | `tests/test_guiclaw_p2_integration.py::test_full_flow_with_mock_llm` |
</phase_requirements>
...
The code surface for the seven requirements is concentrated in `guiclaw/agent.py`. The main tests are already present in `tests/test_guiclaw_p2_integration.py` and `tests/test_guiclaw_p2_memory.py`. Phase 6 remains a dependency because it fixed the embedding-backed host wiring in `nanobot/agent/tools/gui.py`; that dependency is covered by `tests/test_guiclaw_p6_wiring.py`. A live targeted run on the current branch succeeded with `uv run pytest tests/test_guiclaw_p2_integration.py tests/test_guiclaw_p2_memory.py tests/test_guiclaw_p6_wiring.py -q` and produced `15 passed, 3 warnings in 1.91s`.
...
|------|-------|----------------|
| `guiclaw/agent.py` | 191-215 | `run()` starts trajectory recording, retrieves memory, searches skills, switches to skill phase, and falls back to agent phase |
| `guiclaw/agent.py` | 251-255, 343-355 | `finish()` and `record_step()` prove trajectory integration |
| `guiclaw/agent.py` | 539-547 | `build_system_prompt(..., memory_context=memory_context)` is the prompt injection seam |
| `guiclaw/agent.py` | 756-785 | `_retrieve_memory()` formats memory context and always includes `POLICY` entries |
| `guiclaw/agent.py` | 787-801 | `_search_skill()` gates skill execution by relevance × confidence threshold |
...
|------|----------------|--------------|
| `tests/test_guiclaw_p2_integration.py` | `test_memory_injected_into_system_prompt` | `AGENT-04`, `MEM-05` |
| `tests/test_guiclaw_p2_integration.py` | `test_skill_path_chosen_above_threshold` | `AGENT-05`, `SKILL-08` |
| `tests/test_guiclaw_p2_integration.py` | `test_free_explore_when_no_skill_match` | `AGENT-05` |
| `tests/test_guiclaw_p2_integration.py` | `test_trajectory_recorded_on_run` | `AGENT-06`, `TRAJ-03` |
| `tests/test_guiclaw_p2_integration.py` | `test_full_flow_with_mock_llm` | `TEST-05` plus end-to-end corroboration for the other six |
| `tests/test_guiclaw_p2_memory.py` | `test_policy_always_included` | `MEM-05` |
| `tests/test_guiclaw_p2_memory.py` | `test_memory_context_formatted_in_system_prompt` | `MEM-05`, `AGENT-04` |
| `tests/test_guiclaw_p6_wiring.py` | `test_gui_tool_wires_embedding_adapter_when_configured` | Phase 6 dependency evidence for skill-search reachability in the nanobot path |
...
```bash
uv run pytest tests/test_guiclaw_p2_integration.py tests/test_guiclaw_p2_memory.py tests/test_guiclaw_p6_wiring.py -q
```
...
tests/
├── test_guiclaw_p2_integration.py
├── test_guiclaw_p2_memory.py
└── test_guiclaw_p6_wiring.py
```
...
```md
| AGENT-05 | VERIFIED | `guiclaw/agent.py:197-215,787-801` | `test_skill_path_chosen_above_threshold`; `test_free_explore_when_no_skill_match` | Verified at `GuiAgent` layer; wrapper caveat noted separately |
```
...
1. **Should the rewritten Phase 2 verification explicitly call out the nanobot wrapper caveat?**
   - What we know: `guiclaw/agent.py` satisfies the seven requirements and the targeted tests pass.
...
|--------|----------|-----------|-------------------|-------------|
| AGENT-04 | Memory retrieval is injected into the `GuiAgent` system prompt | integration | `uv run pytest tests/test_guiclaw_p2_integration.py::test_memory_injected_into_system_prompt -q` | ✅ |
| AGENT-05 | Agent chooses skill execution when match is above threshold and free explore when not | integration | `uv run pytest tests/test_guiclaw_p2_integration.py::test_skill_path_chosen_above_threshold tests/test_guiclaw_p2_integration.py::test_free_explore_when_no_skill_match -q` | ✅ |
| AGENT-06 | Agent run emits trajectory metadata, steps, and result | integration | `uv run pytest tests/test_guiclaw_p2_integration.py::test_trajectory_recorded_on_run -q` | ✅ |
| MEM-05 | `POLICY` memories are always included and formatted into prompt context | integration | `uv run pytest tests/test_guiclaw_p2_memory.py::test_policy_always_included tests/test_guiclaw_p2_memory.py::test_memory_context_formatted_in_system_prompt -q` | ✅ |
| SKILL-08 | Skill execution branch is integrated into the agent loop | integration | `uv run pytest tests/test_guiclaw_p2_integration.py::test_skill_path_chosen_above_threshold -q` | ✅ |
| TRAJ-03 | Trajectory recording is integrated into the loop and produces JSONL events | integration | `uv run pytest tests/test_guiclaw_p2_integration.py::test_trajectory_recorded_on_run -q` | ✅ |
| TEST-05 | Full DryRun + mock LLM + memory + skill flow passes end-to-end | integration | `uv run pytest tests/test_guiclaw_p2_integration.py::test_full_flow_with_mock_llm -q` | ✅ |

### Sampling Rate

- **Per task commit:** `uv run pytest tests/test_guiclaw_p2_integration.py tests/test_guiclaw_p2_memory.py tests/test_guiclaw_p6_wiring.py -q`
...
- `guiclaw/agent.py` - exact implementation of memory injection, skill search/execution gating, and trajectory recording
- `tests/test_guiclaw_p2_integration.py` - main requirement evidence for `AGENT-04`, `AGENT-05`, `AGENT-06`, `SKILL-08`, `TRAJ-03`, `TEST-05`
- `tests/test_guiclaw_p2_memory.py` - main requirement evidence for `MEM-05`
- `tests/test_guiclaw_p6_wiring.py` - dependency evidence that Phase 6 closed the embedding search seam
...
- Live command result on 2026-03-19: `uv run pytest tests/test_guiclaw_p2_integration.py tests/test_guiclaw_p2_memory.py tests/test_guiclaw_p6_wiring.py -q` -> `15 passed, 3 warnings in 1.91s`
