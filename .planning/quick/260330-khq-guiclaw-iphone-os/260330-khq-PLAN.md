...
must_haves:
  ...
  artifacts:
    - path: "guiclaw/backends/ios_wda.py"
  ...
  key_links:
    - from: "guiclaw/cli.py"
      to: "guiclaw/backends/ios_wda.py"
...
<objective>
Add an iOS/iPhone device control backend to GUIClaw using WebDriverAgent (WDA) via the `facebook-wda` Python client (`wda` package).

Purpose: Enable GUIClaw to automate iOS devices with the same action vocabulary as the existing ADB Android backend.
...
<context>
@guiclaw/backends/adb.py (reference implementation pattern)
@guiclaw/interfaces.py (DeviceBackend protocol)
@guiclaw/cli.py (build_backend, parse_args, AppCache.cache_key)
@guiclaw/skills/normalization.py (Android normalization pattern to mirror for iOS)
@guiclaw/action.py (Action dataclass, VALID_ACTION_TYPES, resolve_coordinate)
@guiclaw/observation.py (Observation dataclass)
...
<task type="auto">
  ...
  <files>guiclaw/backends/ios_wda.py, guiclaw/skills/normalization.py</files>
  <action>
**1a. Create `guiclaw/backends/ios_wda.py`** (~250-350 lines)
...
- Handle all action types matching AdbBackend's coverage:
  - `tap`: `session.tap(x, y)` — resolve coordinates via `resolve_coordinate()` from `guiclaw.action`
...
Coordinate resolution helpers: same `_resolve_x`, `_resolve_y`, `_resolve_point`, `_resolve_second_point` as AdbBackend, using `resolve_coordinate` from `guiclaw.action`.

**1b. Add iOS bundle ID mappings to `guiclaw/skills/normalization.py`**
...
<task type="auto">
  ...
  <files>guiclaw/cli.py, nanobot/agent/router.py, nanobot/agent/loop.py, nanobot/agent/capabilities.py</files>
  <action>
**2a. Update `guiclaw/cli.py`**
...
6. Where `annotate_android_apps` is called to build app list for system prompt, add iOS branch. Find where the app list annotation happens (likely in the run function) and add:
   ...
   if backend.platform == "ios":
       from guiclaw.skills.normalization import annotate_ios_apps
...
from guiclaw.cli import parse_args, build_backend
...
<verification>
1. `python -c "from guiclaw.backends.ios_wda import WdaBackend"` imports without error
2. `python -m guiclaw.cli --backend ios --task "test" --dry-run` does not crash on argument parsing (will fail at WDA connection which is expected)
...
<output>
After completion, create `.planning/quick/260330-khq-guiclaw-iphone-os/260330-khq-SUMMARY.md`
</output>
