...
OpenGUI 是一个基于视觉的 GUI 自动化引擎。它对当前屏幕截图，交由大模型决定下一步操作，执行后再次截图，如此循环直到任务完成。支持 Android（通过 ADB）、桌面端（macOS / Linux / Windows）以及 dry-run 测试模式。
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `backend` | `"adb"` \| `"local"` \| `"dry-run"` | `"adb"` | 执行后端 |
...
修改 `gui` 节中的 `backend` 字段即可切换 Android / 桌面控制：
...
- `platform` 必须是以下之一：`android`、`macos`、`linux`、`windows`、`dry-run`
- 元数据块与正文之间用**空行**分隔
- 同一文件中多个条目以 `##` 标题分隔
