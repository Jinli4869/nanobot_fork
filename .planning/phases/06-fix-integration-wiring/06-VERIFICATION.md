...
**Phase Goal:** Close the three broken integration seams left after Phases 3-5: nanobot embedding search wiring, desktop Pillow packaging, and the installable `guiclaw` console script.
**Verified:** 2026-03-19T12:00:00Z
**Status:** PASSED
...
| 5 | `pyproject.toml` declares `guiclaw = "guiclaw.cli:main"` under `[project.scripts]` | VERIFIED | `pyproject.toml` line 88: `guiclaw = "guiclaw.cli:main"`; `guiclaw/cli.py` line 355 defines `def main(argv: list[str] | None = None) -> int:`; test `test_pyproject_declares_guiclaw_console_script` passes |
...
|----------|----------|--------|---------|
| `tests/test_guiclaw_p6_wiring.py` | Focused regression coverage for the Phase 6 gap-closure contract | VERIFIED | All 5 test functions present and substantive; 5 passed in live run (1.97s) |
...
| `pyproject.toml` | `Pillow>=10.0` in `desktop` and `dev`; `guiclaw = "guiclaw.cli:main"` in `[project.scripts]` | VERIFIED | All three metadata items confirmed at lines 71, 83, and 88 respectively |
...
| `pyproject.toml` | `guiclaw/cli.py` | declares installable console entry point `guiclaw.cli:main` | WIRED | `pyproject.toml` line 88 declares `guiclaw = "guiclaw.cli:main"`; `guiclaw/cli.py` line 355 defines `def main(argv: list[str] | None = None) -> int:` — entry point target exists |
...
| CLI-01 | 06-01-PLAN | `python -m guiclaw.cli` standalone entry point — installable console script component | SATISFIED | `guiclaw = "guiclaw.cli:main"` in `[project.scripts]`; `Pillow>=10.0` also added to `dev` so CI can run desktop tests |
...
| `pyproject.toml` | Placeholder dependency | None | `Pillow>=10.0` is a concrete version-pinned dependency; `guiclaw.cli:main` resolves to a real function |
| `tests/test_guiclaw_p6_wiring.py` | Empty test bodies | None | All 5 test functions contain substantive assertions verified by live run |
...
| `4575db8` | `feat(06-01): wire embedding adapter, add Pillow dep and guiclaw script` — TDD GREEN phase | Confirmed in git log |
...
3. **Installable console script (CLI-01):** `guiclaw = "guiclaw.cli:main"` is added to `[project.scripts]` in `pyproject.toml`. After `pip install`, users can invoke `guiclaw` directly without `python -m guiclaw.cli`.

Five dedicated regression tests in `tests/test_guiclaw_p6_wiring.py` lock all three seams and pass on the current branch.
