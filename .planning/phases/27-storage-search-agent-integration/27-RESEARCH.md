...
| STOR-02 | Unified skill search covers both layers with layer-aware relevance scoring | `SkillLibrary` already uses BM25 + optional FAISS hybrid via `_BM25Index` and `_FaissIndex` from `guiclaw.memory.retrieval`. `UnifiedSkillSearch` wraps both stores, runs search on each, and blends results with a layer-weight factor before returning a ranked `list[SkillSearchResult]`. |
...
2. **Search layer** — `UnifiedSkillSearch` that queries both stores independently, applies per-layer weights to the relevance scores, and returns a ranked union. The BM25 and FAISS index building blocks already exist in `guiclaw.memory.retrieval`; the stores just need to own their own indices.
...
| `guiclaw.skills.shortcut` | workspace current | `ShortcutSkill`, `StateDescriptor`, `ParameterSlot` — the type `ShortcutSkillStore` persists | Phase 24 output, already has `to_dict/from_dict` |
| `guiclaw.skills.task_skill` | workspace current | `TaskSkill`, `TaskNode` — the type `TaskSkillStore` persists | Phase 24 output, already has `to_dict/from_dict` |
| `guiclaw.memory.retrieval._BM25Index` | workspace current | BM25 full-text index for search within each store | Shared module already used by `SkillLibrary` |
| `guiclaw.memory.retrieval._FaissIndex` | workspace current | Optional FAISS embedding index (hybrid search) | Same lazy-import pattern already used by `SkillLibrary` |
| `guiclaw.memory.retrieval.EmbeddingProvider` | workspace current | Optional async embedding protocol for FAISS | Already used by `SkillLibrary`; `GuiSubagentTool` already constructs a `NanobotEmbeddingAdapter` |
| `guiclaw.memory.store.MemoryStore` | workspace current | Memory entry lookup by `entry_id` for INTEG-02 | Existing production class; `MemoryStore.get(entry_id)` returns `MemoryEntry | None` |
| `guiclaw.agent.GuiAgent` | workspace current | The agent being wired — `_search_skill()` updated, `_inject_skill_memory_context()` added | The primary integration point |
...
| `guiclaw.skills.normalization.normalize_app_identifier` | workspace current | App identifier normalization during skill storage | Same hook used by `SkillLibrary._normalize_skill()` |
...
tests/
└── test_guiclaw_p27_storage_search_agent.py  # NEW: Phase 27 coverage
```
...
**Example:**
```python
# Source: guiclaw/skills/library.py SkillLibrary._save_platform()
...
**Example:**
```python
# Source: guiclaw/skills/library.py SkillLibrary.search() and _rebuild_index()
...
**Example:**
```python
# Source: GuiAgent.run() structure from guiclaw/agent.py, MemoryStore.get() from guiclaw/memory/store.py
...
async def _inject_skill_memory_context(
    ...
    from guiclaw.skills.task_skill import TaskSkill as _TaskSkill
...
**Example:**
```python
# Source: guiclaw/skills/library.py SkillLibrary._skill_text()
@staticmethod
...
| BM25 text index | Custom inverted index | `guiclaw.memory.retrieval._BM25Index` | CJK-aware (jieba), battle-tested, shared across `SkillLibrary` and `MemoryRetriever` |
| FAISS embedding index | Custom vector store | `guiclaw.memory.retrieval._FaissIndex` | Already wrapped for lazy-import; in-session rebuild is acceptable at current scale (deferred persistent FAISS per REQUIREMENTS.md Out of Scope) |
| Min-max score normalization | Custom normalization | `_min_max_norm()` from `guiclaw/skills/library.py` — either copy the 10-line function or factor it to `guiclaw/memory/retrieval.py` | Already handles edge cases (empty mask, zero-range normalization) |
...
```python
# Source: guiclaw/skills/library.py _save_platform / load_all pattern
# Combined with ShortcutSkill.to_dict() / from_dict() from guiclaw/skills/shortcut.py
...
```python
# Source: guiclaw/agent.py GuiAgent._search_skill() — to be replaced in Phase 27
...
```python
# Source: guiclaw/agent.py GuiAgent.__init__() — add these two parameters
def __init__(
    self,
    ...
...
```python
# Source: guiclaw/memory/store.py MemoryStore.get() — returns MemoryEntry | None
entry = self._memory_store.get(skill.memory_context_id)
...
3. **Where should `_min_max_norm()` live to be shared between `SkillLibrary` and `ShortcutSkillStore`/`TaskSkillStore`?**
   - What we know: It is currently a module-level function in `guiclaw/skills/library.py`. Both new stores will need it.
   - What's unclear: Whether to duplicate it (simple but fragile) or move it to a shared location.
   - Recommendation: Copy the 10-line function into `shortcut_store.py` to avoid coupling to `library.py`. If it grows beyond that, factor it to `guiclaw/memory/retrieval.py` as a proper utility. Premature factoring is not worth the refactoring risk here.
...
| Quick run command | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q` |
| Full suite command | `uv run pytest -q` |

### Phase Requirements -> Test Map
| Req ID | Behavior | Test Type | Automated Command | File Exists? |
|--------|----------|-----------|-------------------|-------------|
| STOR-01a | `ShortcutSkillStore.add()` then reload from disk produces identical `ShortcutSkill` objects | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k shortcut_store_round_trip` | ❌ Wave 0 |
| STOR-01b | `TaskSkillStore.add()` then reload from disk produces identical `TaskSkill` objects | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k task_store_round_trip` | ❌ Wave 0 |
| STOR-01c | JSON file carries `"version": 1` envelope field | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k version_field` | ❌ Wave 0 |
| STOR-02 | `UnifiedSkillSearch.search()` returns results from both layers ranked by score with layer label | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k unified_search` | ❌ Wave 0 |
| INTEG-01 | `GuiAgent._search_skill()` returns `SkillSearchResult` with `layer` field; log entry includes layer | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k agent_skill_lookup` | ❌ Wave 0 |
| INTEG-02 | When selected skill is `TaskSkill` with `memory_context_id`, injected content appears in `memory_context` before `_run_once` | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k memory_context_injection` | ❌ Wave 0 |
| INTEG-02 guard | Missing `memory_context_id` in `MemoryStore` logs a warning and returns `existing_context` unchanged | unit | `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py -q -k missing_memory_context` | ❌ Wave 0 |
| Phase 27 import safety | New module imports and compiles without circular imports | smoke | `uv run python -m py_compile guiclaw/skills/shortcut_store.py` | ❌ Wave 0 |

### Sampling Rate
- **Per task commit:** `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py tests/test_guiclaw_p26_quality_gated_extraction.py tests/test_guiclaw_p24_schema_grounding.py -q`
- **Per wave merge:** `uv run pytest tests/test_guiclaw_p27_storage_search_agent.py tests/test_guiclaw_p25_multi_layer_execution.py tests/test_guiclaw_p26_quality_gated_extraction.py tests/test_guiclaw_p24_schema_grounding.py -q`
- **Phase gate:** `uv run pytest -q`

### Wave 0 Gaps
- [ ] `tests/test_guiclaw_p27_storage_search_agent.py` — covers STOR-01, STOR-02, INTEG-01, INTEG-02, and import safety
- [ ] `guiclaw/skills/shortcut_store.py` — new module with `ShortcutSkillStore`, `TaskSkillStore`, `UnifiedSkillSearch`, `SkillSearchResult`
- [ ] Export `ShortcutSkillStore`, `TaskSkillStore`, `UnifiedSkillSearch`, `SkillSearchResult` from `guiclaw/skills/__init__.py`
...
- `guiclaw/skills/library.py` — `SkillLibrary` persistence pattern (atomic write, BM25/FAISS hybrid, load_all); the definitive template for both new store classes
- `guiclaw/skills/shortcut.py` — `ShortcutSkill`, `StateDescriptor`, `ParameterSlot` schemas with `to_dict/from_dict` — confirmed round-trip contracts
- `guiclaw/skills/task_skill.py` — `TaskSkill`, `TaskNode` schemas with `to_dict/from_dict` — confirmed round-trip contracts, `memory_context_id` opaque pointer
- `guiclaw/memory/retrieval.py` — `_BM25Index`, `_FaissIndex`, `EmbeddingProvider` — shared search building blocks
- `guiclaw/memory/store.py` — `MemoryStore.get(entry_id)` — INTEG-02 entry lookup mechanism; atomic write pattern
- `guiclaw/memory/types.py` — `MemoryEntry.content` field — the text injected as memory context
- `guiclaw/agent.py` — `GuiAgent.__init__()` parameter list, `run()` step ordering, `_search_skill()`, `_retrieve_memory()`, `_skill_maintenance()` — full agent wiring context
...
- `guiclaw/skills/__init__.py` — current `__all__` export list; reference for adding Phase 27 symbols
- `guiclaw/skills/shortcut_extractor.py` — Phase 26 output confirming `ShortcutSkill` is the store's input type
- `tests/test_guiclaw_p25_multi_layer_execution.py` — test stub style (`_FakeBackend`, `_StubGrounder`) confirms the fake-dependency test pattern for Phase 27 tests
