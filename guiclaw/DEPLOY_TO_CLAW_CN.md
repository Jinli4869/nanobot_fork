# GUIClaw 迁移部署指南（面向 OpenClaw / NanoClaw / 其他 Claw 宿主）

本指南面向这样一种场景：

- 你希望把 `guiclaw` 单独迁移到 `openclaw`、`nanoclaw` 或其他 `*claw` 宿主中
- 你希望不同宿主使用一套尽量统一的 GUI 配置
- 你希望通过最少代码接入，就让宿主快速获得 GUI 操控能力

这份文档不重复介绍 GUIClaw 的基础 CLI 用法，重点解释宿主如何嵌入、配置、运行，以及如何决定是否接入任务结束后的总结、评测、技能沉淀等后处理能力。
...
2. 宿主适配层
   - 把宿主自己的 LLM Provider 适配成 GUIClaw 期望的接口
   - 把宿主自己的配置映射到 GUIClaw 的统一配置
   - 把 GUI 结果包装成宿主工具返回值
   - 决定是否在任务结束后执行后处理
...
- `provider` / `model`
  - 指向宿主自己的 LLM 配置，不要求 GUIClaw 自己管理多提供商
- `agentProfile`
...
`GuiAgent` 需要一个满足 GUIClaw LLM 协议的对象，最小能力是：
...
如果宿主已经有自己的 Provider，推荐像 nanobot 一样写一层适配器，而不是改 GUIClaw 本体。
...
- `adb` -> `guiclaw.backends.adb.AdbBackend`
- `ios` -> `guiclaw.backends.ios_wda.WdaBackend`
- `hdc` -> `guiclaw.backends.hdc.HdcBackend`
- `local` -> `guiclaw.backends.desktop.LocalDesktopBackend`
- `dry-run` -> `guiclaw.backends.dry_run.DryRunBackend`

可参考：

- [nanobot/agent/tools/gui.py](/Users/jinli/Documents/Personal/nanobot_fork/nanobot/agent/tools/gui.py)
- [guiclaw/cli.py](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/cli.py)
...
最小落地方案推荐使用“宿主工具包装器”模式，而不是直接在宿主主循环里散落调用 GUIClaw。
...
- 把宿主 Provider 包装成 GUIClaw LLM 接口
- 可选包装 embedding 接口
...
## 7. 统一配置到 GUIClaw 的映射关系

建议所有 claw 使用同一套映射表：

| 宿主统一字段 | GUIClaw 内部字段 | 说明 |
|---|---|---|
...
建议不要把总结逻辑硬编码在 `guiclaw` 本体里，而是在宿主中做开关：
...
建议不要把 GUIClaw 的宿主接入代码散在多个业务目录中。
...
或把 GUIClaw 作为 monorepo 内的可编辑依赖。
...
### 14.2 直接把宿主 Provider 塞给 GUIClaw
...
如果你现在要把 GUIClaw 快速迁到 `openclaw` / `nanoclaw`：
...
- [guiclaw/agent.py](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/agent.py)
- [guiclaw/cli.py](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/cli.py)
- [guiclaw/trajectory/recorder.py](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/trajectory/recorder.py)
- [guiclaw/trajectory/summarizer.py](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/trajectory/summarizer.py)
- [nanobot/agent/gui_adapter.py](/Users/jinli/Documents/Personal/nanobot_fork/nanobot/agent/gui_adapter.py)
- [nanobot/agent/tools/gui.py](/Users/jinli/Documents/Personal/nanobot_fork/nanobot/agent/tools/gui.py)
