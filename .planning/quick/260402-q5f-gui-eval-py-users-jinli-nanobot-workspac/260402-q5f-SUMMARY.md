...
- 在 `guiclaw/agent.py` 中为非终止动作加入固定的 post-action settle 延迟；`_run_step()` 现在按 `execute -> settle -> observe` 顺序推进。
...
- `uv run pytest tests/test_guiclaw.py -q -k settle_before_observing`
- `uv run pytest tests/test_guiclaw_p8_trajectory.py -q -k "counts_only_step_rows or uses_step_only_counts"`
- `uv run pytest tests/test_guiclaw.py tests/test_guiclaw_p8_trajectory.py -q`
- 实际 trace 校验：
