...
async def _run_probe(task: str, os_path: Path, app_path: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="guiclaw-memory-probe-") as tmp:
        tmp_path = Path(tmp)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe GUIClaw memory retrieval using raw guide markdown files.")
    parser.add_argument("task", help="Task/query to test against memory retrieval.")
    parser.add_argument(
        "--os-guide",
        type=Path,
        default=Path("/Users/jinli/.guiclaw/memory/os_guide.md"),
        help="Path to raw os_guide markdown.",
    )
    parser.add_argument(
        "--app-guide",
        type=Path,
        default=Path("/Users/jinli/.guiclaw/memory/app_guide.md"),
