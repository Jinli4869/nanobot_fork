...
plan: 01
subsystem: cli
tags: [guiclaw, cli, argparse, yaml, openai-compatible, dry-run, pytest]
requires:
...
provides:
  - Standalone `python -m guiclaw.cli` entry point with YAML config loading and backend selection
  - OpenAI-compatible chat and embedding adapters inside `guiclaw` runtime code
...
key-files:
  created: [.planning/phases/05-cli-extensions/05-01-SUMMARY.md, guiclaw/cli.py, guiclaw/__main__.py, tests/test_guiclaw_p5_cli.py]
  modified: [pyproject.toml]
key-decisions:
  - "The standalone CLI owns its config schema and OpenAI-compatible provider bridge so `guiclaw` never imports nanobot runtime code."
...
patterns-established:
  - "CLI entry points in `guiclaw` should assemble existing runtime pieces and delegate execution to `GuiAgent.run()`."
...
**Standalone `guiclaw` CLI with YAML config loading, OpenAI-compatible provider wiring, backend selection, and optional memory/skills bundle**
...
- Added [`guiclaw/cli.py`](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/cli.py) with argparse parsing, config loading from `~/.guiclaw/config.yaml`, backend factory selection, OpenAI-compatible chat and embedding adapters, and `GuiAgent` run orchestration.
- Added [`guiclaw/__main__.py`](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/__main__.py) so the package has a direct module entry point that delegates to the CLI.
- Added [`tests/test_guiclaw_p5_cli.py`](/Users/jinli/Documents/Personal/nanobot_fork/tests/test_guiclaw_p5_cli.py) to lock the CLI contract: task parsing, config/env fallback, backend variants, dry-run execution, JSON output, module delegation, and optional memory/skills bundle gating.
...
- [`guiclaw/cli.py`](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/cli.py) - Standalone CLI parser, YAML config loader, provider bridges, backend factory, and `GuiAgent` wiring.
- [`guiclaw/__main__.py`](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/__main__.py) - Minimal package entry point delegating to `guiclaw.cli.main`.
- [`tests/test_guiclaw_p5_cli.py`](/Users/jinli/Documents/Personal/nanobot_fork/tests/test_guiclaw_p5_cli.py) - Regression coverage for the full Phase 5 CLI contract.
- [`pyproject.toml`](/Users/jinli/Documents/Personal/nanobot_fork/pyproject.toml) - Adds the runtime YAML dependency needed by `~/.guiclaw/config.yaml`.

## Decisions Made

- Kept the provider and embedding adapters inside `guiclaw/cli.py` so the CLI stays self-contained and runtime-independent from nanobot.
...
- Provide `~/.guiclaw/config.yaml` with `provider.base_url` and `provider.model`.
...
- [x] [`guiclaw/cli.py`](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/cli.py) exists
- [x] [`guiclaw/__main__.py`](/Users/jinli/Documents/Personal/nanobot_fork/guiclaw/__main__.py) exists
- [x] [`tests/test_guiclaw_p5_cli.py`](/Users/jinli/Documents/Personal/nanobot_fork/tests/test_guiclaw_p5_cli.py) exists
