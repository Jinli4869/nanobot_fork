...
@guiclaw/agent.py (lines 492-541 — GuiAgent.__init__, lines 570-725 — run() skill/shortcut block)
@guiclaw/postprocessing.py (lines 118-200 — _run_all and _promote_shortcut)
</context>
...
<task type="auto">
  ...
  <files>nanobot/agent/tools/gui.py, guiclaw/agent.py</files>
  <action>
...
2. Inside the `if self._gui_config.enable_skill_execution:` block (line 232):
   - Remove the imports of `ShortcutExecutor`, `LLMConditionEvaluator` (from `guiclaw.skills.multi_layer_executor`), and `ShortcutApplicabilityRouter` (from `guiclaw.skills.shortcut_router`).
...
**In `guiclaw/agent.py` (lines 570-725 in the `run()` method):**
...
with open('guiclaw/agent.py') as f:
    agent_src = f.read()
...
<task type="auto">
  ...
  <files>guiclaw/postprocessing.py</files>
  <action>
**In `guiclaw/postprocessing.py`:**
...
1. Replace the `_promote_shortcut` method (lines 154-200) with a simpler `_extract_skill` method that:
   ...
   - Imports only `SkillExtractor` from `guiclaw.skills.extractor` (NOT ShortcutPromotionPipeline, NOT ShortcutSkillStore)
...
Note: The `_legacy_skill_to_shortcut` static method (line 282+) can remain in the file — it will simply be dead code. Do not delete it to minimize diff size.
  ...
  <verify>
    <automated>cd /Users/jinli/Documents/Personal/nanobot_fork && python -c "
import ast
with open('guiclaw/postprocessing.py') as f:
    src = f.read()
tree = ast.parse(src)
...
python -m py_compile guiclaw/agent.py
python -m py_compile guiclaw/postprocessing.py
