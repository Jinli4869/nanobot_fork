...
plan: "01"
subsystem: guiclaw-backends
tags: [hdc, harmonyos, backend, guiclaw, nanobot, device-automation]
dependency_graph:
  requires:
    - guiclaw/backends/adb.py (pattern reference for async subprocess)
    - guiclaw/backends/ios_wda.py (pattern reference for iOS integration)
    - guiclaw/interfaces.py (DeviceBackend protocol)
    - guiclaw/action.py (Action, resolve_coordinate, describe_action)
    - guiclaw/observation.py (Observation dataclass)
  provides:
    - guiclaw/backends/hdc.py (HdcBackend, HdcError)
    - guiclaw/cli.py (HdcConfig, hdc backend choice and config loading)
  ...
  affects:
    ...
    - guiclaw CLI --backend hdc invocations
tech_stack:
  added:
...
### Task 1: HdcBackend (guiclaw/backends/hdc.py)
...
Files exist:
- `guiclaw/backends/hdc.py`: FOUND
- `.planning/quick/260330-l0g-guiclaw-hdc-harmony-os/260330-l0g-SUMMARY.md`: FOUND
