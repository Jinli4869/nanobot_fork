...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `backend` | `"adb"` \| `"scrcpy-adb"` \| `"ios"` \| `"hdc"` \| `"local"` \| `"dry-run"` | `"adb"` | 执行后端 |
...
```json
"gui": { "backend": "adb"        }   // Android 设备（ADB 截图与操作）
"gui": { "backend": "scrcpy-adb" }   // Android scrcpy 帧 + ADB 操作
...
agent 输出、`gui_task` 工具调用、OpenGUI 动作和最终 trace 元数据。静态 demo
轨迹仍然保留。
...
- **Android**：中间屏幕使用 `py-scrcpy-sdk` 提供的 scrcpy 视频流；GUI 执行使用所选后端（默认 `adb`，或 `scrcpy-adb` 表示 scrcpy 观察 + ADB 操作）。
...
| `adb` 或 `scrcpy-adb` | `gui.adb` | Android 设备（ADB 兼容控制） |
...
"backend": "scrcpy-adb" → planner 生成 route_id = "gui.adb"
